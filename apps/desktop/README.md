...
- **パラメータ走査**: 各スライダーの「走査」ボタンで、そのパラメータだけを
  6 段階に変えた比較レンダリングをプレビュー上に一覧表示
  （`start_sweep` コマンド → `sweep_result` イベント）。
  サムネイルをクリックすると値がスライダーに反映される。
  比較用レンダリングは拡大なし（out_long = resolution）で高速化
