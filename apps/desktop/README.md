...
- **顔検出（人物向け）**: 「顔を検出して強化」で UltraFace（ONNX）が顔を
  自動検出し、顔領域のディテールを強化（目・鼻・口の潰れを軽減）。
  models/ultraface_rfb_320.onnx を自動検出、無ければファイル選択
  （load_face_model コマンド）。強さは「顔ディテール強化」スライダー
