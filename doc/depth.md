...
パレット吸着済みターゲットの色替わり箇所を ±2px スパンの色コントラストで
重み付けして抽出する——単純な境界抽出だと滑らかなグラデーションの減色
バンディング（床や空の等高線状ノイズ）まで拾うため、色差の大きい
「本物の輪郭」だけを残す。インクは局所色を 25% に落としたもの（黒より馴染む）。
ボケ領域では焦点重みに応じて線も薄くなり、被写界深度の表現と整合する。
中間画像 `line_art.png` で抽出結果を確認できる。
